...
public:
  template <class T>
  CUDAIN static constexpr T load(const atomic_type<T>& a) {
    return a;
  }

  template <class T>
  CUDAIN static constexpr std::enable_if_t<!read_only, void> store(atomic_type<T>& a, T v) {
    a = v;
  }

  template <class T>
  CUDAIN static constexpr std::enable_if_t<!read_only, T> exchange(atomic_type<T>& a, T v) {
...
public:
  ...
  template <class T>
  CUDAIN static T load(const atomic_type<T>& a) {
    return a.load(mem_order);
  }

  template <class T>
  CUDAIN static void store(atomic_type<T>& a, T v) {
    a.store(v, mem_order);
  }

  template <class T>
  CUDAIN static T exchange(atomic_type<T>& a, T v) {
